...
% [salida] = sintetizador_guitarra(f, n_nota, n_ruido_blanco, fs)
% Entradas:
...
function [salida] = sintetizador_guitarra(f, n_nota, n_ruido_blanco, fs)
...
entrada = [rand(1, n_ruido_blanco) - 0.5, zeros(1, 3*fs)]; % Ruido blanco a la entrada
nota = filter(B, A, entrada); % Filtramos la entrada por el sistema para obtener la nota

% Mejora #2 para el decaimiento: Decaimiento exponencial al final de la nota para evitar que finalice de forma abupta y antinatural
