...
if seleccion == 1 % Canción 1: Cumpleaños Feliz
    ...
    octavas = 2;
    notas = [si fa si fa si fa si do mi re do sol la re fa si la fa_s];
...
prompt = 'Introduzca el número de pulsaciones por minuto (mínimo 30): ';
bpm = input(prompt); % pulsaciones por minuto
while bpm < 30
        prompt = 'El mínimo de pulsaciones por minuto es 30\nIntroduzca el número de pulsaciones por minuto: ';
        bpm = input(prompt);
end
...
n_cancion = max(ini_muestras) + n_ruido_blanco + 3*fs; % Longitud de la canción (en número de muestras)
...
for i = 1:length(notas) % Para cada nota
	...
	nota = sintetizador_guitarra((2^(octava - 1))*notas(i), n_nota, n_ruido_blanco, fs); % Se sintetiza la nota
